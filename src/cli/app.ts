...
export function parseSlashCommand(input: string): SlashCommand | undefined {
  ...
  if (command === "model") {
    ...
    if (parts[1] === "list") return { type: "model", action: "list" };
...
export function renderHelp(): string {
  return [
    ...
    "  /tasks     List, inspect, message, create, update, stop, output, or delete tasks",
    "  /teams     List, inspect, create, message, run, or delete task teams",
    "  /crons     List, inspect, create, target teams, disable, run, or delete cron entries",
    ...
    "  /model     Show, list, switch, or configure the active model",
